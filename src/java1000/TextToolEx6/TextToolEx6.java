	...
	private void registerEventHandler() {
		...
		btn[n++].addActionListener(new ActionListener() { // substring - 문자열 자르기
			public void actionPerformed(ActionEvent ae) {
				...
				String tfString1 = tfParam1.getText();
				String tfString2 = tfParam2.getText();
				int tf1Len = tfString1.length();	//앞에서 삭제~
				int tf2Len = tfString2.length();	//뒤에서 삭제~
				
				Scanner scan = new Scanner(curText);
				while(scan.hasNextLine()) {
					String line = scan.nextLine();
//					line = line.substring(0, tf1Len+1);
					line = line.substring(tf1Len, line.length());
					System.out.println(tf1Len);
					System.out.println(line.length());
					System.out.println(line);
//					line = line.substring(line.length() - tf2Len, line.length());
					//뒤에꺼 삭제. 앞에꺼를 남겨둬야하니까 서브스트링은 0부터.
					line = line.substring(0, line.length() - tf2Len);
 ...
 * 테스트 데이터
 * 
 * sb.append("aaaaaa"); sb.append("bbbbbbbbbbbbb"); sb.append("cccc");
